...
system("cut -d',' -f11 ../Processed-data-schizont/Tp-All.csv > tmp.txt");
system("cut -d';' -f1 tmp.txt > Tp-Motif.txt");
system("rm tmp.txt");
open(B,"Tp-Motif.txt");
...
my@data_Nonp = <A>;
my@slice_data_Nonp = @data_Nonp[1000..1075];
my@data_Pho  = <B>;
...
#print scalar@data_Nonp,"\n";
#print scalar@slice_data_Nonp,"\n";
#print scalar@NonPho_motif,"\n";
#print scalar@slice_data_Nonp_unique,"\n";



##################
my@AA_Pho_freq = Theonine_Array_OUT_File(\@slice_data_Nonp_unique,\@data_Pho);
Theonine_Matix_OUT_File(\@slice_data_Nonp_unique,\@data_Pho,\@AA_Nonp_freq,\@AA_Pho_freq);

########### Motif Correction ###########################################
print scalar@data_Pho,"\n";
Tmotif13mer(@data_Pho);
...
sub Theonine_Array_OUT_File
{
	my($slice_data_Nonp_unique,$data_Pho) = @_;
	open(A,">Phopho-T-Array.csv");
	open(B,">Phopho-T-Array_Relative.csv");
...
sub Theonine_Matix_OUT_File
{
	my($slice_data_Nonp_unique,$data_Pho,$AA_Nonp_freq,$AA_Pho_freq) = @_;
	open(A,">Phopho-T-InD.csv");
	open(B,">Phopho-T-InD_Relative.csv");
	my@AA = ("A","C","D","E","F","G","H","I","K","L","M","N","P","Q","R","S","T","W","Y","V");
